...
- `install_plugin_to_codex.ps1`
  Copies the plugin bundle into `~/.codex/local-marketplaces/<marketplace-name>/plugins/codex-review`, also copies it into `~/.codex/plugins/cache/<marketplace-name>/codex-review`, writes `.agents/plugins/marketplace.json`, and enables the plugin in `~/.codex/config.toml` so Codex Desktop can discover and load it.
