...
3. Optional local lessons snapshots that are reviewed and distilled into repo-local prompts deliberately, not auto-ingested raw
...
The optional lessons-snapshot lane is a maintainer input, not a public benchmark lane. It exists to refresh the reviewer's bug-hunting prompts from repeated real-world mistakes without copying a local notes source directly into the durable public corpus.
